...
// 1. Remove from vocabN2N1.ts
const n2Path = path.join(process.cwd(), 'src', 'data', 'vocabN2N1.ts');
let n2Content = fs.readFileSync(n2Path, 'utf8');

// Match everything from { id: 'v_adv_n2_1' to the end of the array.
// But wait, it's safer to just filter out objects that have id starting with 'v_adv_n2_'
let removedAdverbs = [];
let cleanedN2Content = n2Content.replace(/\{\s*id\s*:\s*['"]v_adv_n2_\d+['"][\s\S]*?exampleSentence\s*:\s*\{[\s\S]*?\}\s*\}(?:,\s*)?/g, (match) => {
  // Capture the match if we wanted to reuse it, but we can just parse or recreate it.
  return '';
});

// Clean up trailing commas before ];
cleanedN2Content = cleanedN2Content.replace(/,\s*\];/g, '\n];');
fs.writeFileSync(n2Path, cleanedN2Content, 'utf8');


// 2. Add to vocabN3.ts
// I'll reuse the array from the previous script and change the target to N3
const n2Adverbs = [
...
const n3Path = path.join(process.cwd(), 'src', 'data', 'vocabN3.ts');
let n3Content = fs.readFileSync(n3Path, 'utf8');
n3Content = n3Content.replace(/\];?\s*$/, '');

if (n3Content.trim().length > 100 && !n3Content.trim().endsWith(',')) {
   n3Content += ',\n';
}

n2Adverbs.forEach((item, index) => {
  let idPrefix = 'v_adv_n3_';
...
n3Content += `];\n`;
fs.writeFileSync(n3Path, n3Content, 'utf8');

console.log("Moved 56 adverbs from N2 to N3!");
