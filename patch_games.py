...
with open('src/components/GamesHub.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = re.sub(r'\[\s*\{\s*id:\s*\'memory\'.*?\]', new_tabs, content, flags=re.DOTALL)
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-stone-200 shadow-sm max-w-xl mx-auto text-center space-y-6 animate-fadeIn">
          <div className="flex items-center justify-between border-b border-stone-100 pb-4">
            ...
            <div className="flex items-center gap-4 text-xs font-bold">
              ...
              <span className="text-emerald-600">Acertos: {sdScore / 15}</span>
            </div>
          </div>
...
with open('src/components/GamesHub.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
